...
ZbxSender::ZbxSender(const char * configpath)
{
	...
	logger = spdlog::daily_logger_mt("daily_logger", "monitor/zbx_monitor.log", 2, 30);
	...
	ready = true;
    worker_thread_ptr_ =  std::make_shared<std::thread>(&ZbxSender::run, this);
}

void ZbxSender::send(const std::string& data)
{
	queue.push(data);
}
...
{
	stop();
    if (worker_thread_ptr_->joinable())
...
bool ZbxSender::tcp_send(std::string value)
{
    ...
    auto jsonstr = j.dump(-1, ' ', false, nlohmann::json::error_handler_t::ignore);
	...
	std::cout << "josn_msg  len is "<< jsonstr.length() << "content is ["  << jsonstr << "]" << std::endl;
